...
class PlanPostTemplates extends Component {
  constructor() {
    super()
  ...
  render() {
    ...
    const otherPlanPostTemplates = channelPostTemplates.filter((p) => !currentPostTemplate || p.id !== currentPostTemplate.id)
console.log(channelPostTemplates);*/
    ...
    return (
        ...
        <div key={currentPostTemplate.id}>
          ...
          />
          <Button style="inverted" onClick={this.removePostTemplate.bind(this, currentPostTemplate)}>Destroy PostTemplate</Button>
        </div>
    )
  }
}
...
const ConnectedPlanPostTemplates = connect(mapStateToProps, mapDispatchToProps)(PlanPostTemplates)
export default ConnectedPlanPostTemplates
